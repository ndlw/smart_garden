...
const int cthtBegin = 18;
const int cthtEnd = 19;
...
// const char *ssid = "vtb.poro";
// const char *password = "11111111";

// Thông tin MQTT Broker kết nối
// char *mqttServer = "smartgarden.cloud.shiftr.io";
char *mqttServer = "mqtt-dashboard.com";
int mqttPort = 1883;
const char *mqttUser = "smartgarden";
const char *mqttPassword = "Qhso0aAn9XUrGkYN";
...
void callback(char *topic, byte *message, unsigned int length) // Manual
{
  String Strmessage;
  ...
  else if (String(topic) == "esp32/curtain" && Strmessage == "CURTAIN STOP") {
    dungRem();
  } 
...
void reconnect()
{
  ...
  while (!mqttClient.connected())
  {
    ...
    if (mqttClient.connect("ESP32send", mqttUser, mqttPassword))
    {
      ...
      // mqttClient.subscribe("esp32/cthtBegin");
      // mqttClient.subscribe("esp32/cthtEnd");
    }
  }
}
...
void setup()
{
  ...
  pinMode(cthtBegin,INPUT_PULLUP);
  pinMode(cthtEnd,INPUT_PULLUP);
  setupMQTT();
}
...
void docGiaTriDHT() {
  ...
  Serial.print("\n");
  Serial.println("Độ ẩm không khí: " + String(h) + "%");
...
int docGiaTriDoAmDat() {
  ...
  Serial.println("Độ ẩm đất: " + String(doAmDat) + "%");
  return doAmDat;
}

int docGiaTriCthtBegin() {
  statusCthtBegin = digitalRead(cthtBegin);
  if (isnan(statusCthtBegin)) {
    // delay(500);
    Serial.println("Failed to read valid data!\n");
  }
  Serial.println("CTHT Đầu: " + String(statusCthtBegin));
  return statusCthtBegin;
}

int docGiaTriCthtEnd() {
  statusCthtEnd = digitalRead(cthtEnd);
  if (isnan(statusCthtEnd)) {
    // delay(500);
    Serial.println("Failed to read valid data!\n");
  }
  Serial.println("CTHT Cuối: " + String(statusCthtEnd));
  return statusCthtEnd;
...
void changeMode(){
  if (mode == 0) { //mode Auto
    ...
    if (light == 0 && statusCthtBegin == 0) {  // nếu trời mưa và rèm chưa kéo thì kéo rèm
      cheRem();
    Serial.println("Rèm đã che");
    } else {
    if (light == 1 && statusCthtEnd == 0) {
      thuRem();
      Serial.println("Rèm đang thu"); 
    } else { 
    if ((light == 1 && statusCthtBegin == 0) || (light == 0 && statusCthtEnd == 0)){
      dungRem();
    Serial.println("Rèm đã dừng"); }
    }
  }
}
}
...
String create_Json(){
  ...
  String ctht_begin_status = String(docGiaTriCthtBegin());
  String ctht_end_status = String(docGiaTriCthtEnd());
  //,\"Ctht_Begin_Status\": \"" + ctht_begin_status + "\",\"Ctht_End_Status\": \"" + ctht_end_status + "\"
...
String create_Ctht_Json(){
  String ctht_begin_status = String(docGiaTriCthtBegin());
  String ctht_end_status = String(docGiaTriCthtEnd());
  String json = "{\"Ctht_Begin_Status\": \"" + ctht_begin_status + "\",\"Ctht_End_Status\": \"" + ctht_end_status + "\"}";
  return json;
}
...
void loop()
{
  ...
  if (now - previous_time > 150)
  {
    ...
    String Ctht_Json = create_Ctht_Json();
    mqttClient.publish("esp32/json", Json.c_str()); // Topic
    // mqttClient.publish("ctht/json", Ctht_Json.c_str()); // Topic
  }
}